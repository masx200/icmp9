...
function generateXrayConfigipv6() {
  // 读取online.json获取国家代码
...
function generateXrayConfigipv4() {
  // 读取online.json获取国家代码
  const onlineData = JSON.parse(
    fs.readFileSync(path.join(__dirname, "online.json"), "utf8"),
  );
  ...
  async function generateConfig() {
    ...
    try {
      ...
      // 输出新配置文件
      const outputPath = path.join(__dirname, "xray-output-random-ipv4.json");
      await fs.promises.writeFile(outputPath, JSON.stringify(config, null, 2));
      ...
      try {
        const linkContent = await fs.promises.readFile(
          path.join(__dirname, "分享链接-ipv4.txt"),
          "utf8",
        );
        const linkCount = linkContent.trim().split("\n").length;
        console.log(`分享链接.txt中的链接数量: ${linkCount}`);
...
if (import.meta.main) {
  generateXrayConfigipv6();

  generateXrayConfigipv4();
}
