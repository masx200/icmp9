...
// 2. 验证 DNS 映射
console.log('\n2. 检查 DNS 强制映射...')
if (content.includes('"fresh-reverse-proxy-middle.masx201.dpdns.org": "104.21.9.230"')) {
...
async function testFetch() {
  try {
    console.log('\n尝试访问测试域名...')
    const response = await fetch('https://fresh-reverse-proxy-middle.masx201.dpdns.org/', {
