  ...
  for (const url of testUrls) {
    try {
      ...
      if (url.includes('httpbin.org/ip')) {
        ...
        console.log(`[结果] 响应状态: ${response.status} ${response.statusText}`)
        console.log(`[信息] 响应大小: ${response.headers.get('content-length')} bytes`)
      }
...
async function testConfigurableAgent() {
  ...
  try {
    ...
    if (response.ok) {
      const data = await response.json()
      console.log(`[成功] GitHub用户: ${data.login}, 关注者: ${data.followers}`)
    } else {
